...
if st.session_state.game_phase == "player_place_ship":
    ...
    # Automatically trigger computer's turn logic after a small delay for user to read message
    st.session_state.game_phase = "player_turn" # Prepare for next player turn
    st.experimental_rerun() # Trigger a rerun to execute computer_turn() on next cycle
...
# Handle the computer's turn in a separate block to ensure it runs after player's action
if 'game_phase' in st.session_state and st.session_state.game_phase == "computer_turn_processing":
    computer_turn()

# Force a rerender after player's turn to show message and then trigger computer's move
if 'game_phase' in st.session_state and st.session_state.game_phase == "player_turn" and st.session_state.current_turn > 0 and not st.session_state.game_over:
    # If the player just made a move, set phase for computer to process
    if st.session_state.game_phase == "player_turn" and st.session_state.current_turn > 0: # This check is to prevent infinite reruns on initial load
        st.session_state.game_phase = "computer_turn_processing"
